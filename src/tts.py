...
def silero_tts(request: TTSRequest):
    ...
    # return response
    clean_preprocessed_file(input_filename)
    return response_and_delete(input_filename)


def cleanup():
    os.remove(output + "file_" + speaker + "_0.wav")


def response_and_delete(input_filename):
    return FileResponse(input_filename + "_" + speaker + "_0.wav", background=BackgroundTask(cleanup))


def clean_preprocessed_file(input_filename):
    info_file = input_filename + "_preprocessed.txt"
    print("start delete: " + info_file)
    try:
        yield info_file
    finally:
        os.remove(info_file)
